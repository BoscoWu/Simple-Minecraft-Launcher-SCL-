...
class SimpleMCLauncher:
    ...
    def check_for_updates(self):
        ...
        def _check():
            try:
                ...
                # 比较版本（假设版本号格式为 v1.2.3 或 1.2.3）
                current = VERSION_OF_LAUNCHER.lstrip('v')
                latest = latest_tag.lstrip('v')

                if self._compare_versions(latest, current) > 0:
                    self.log(f"发现新版本: {latest_tag} (当前 {VERSION_OF_LAUNCHER})")
                    # 弹出询问对话框
                    self.root.after(0, lambda: self.prompt_update(latest_tag, latest_info))
                else:
                    self.log("当前已是最新版本")
            except Exception as e:
                self.log(f"检查更新失败: {e}")
    ...
    def _get_latest_release_info(self):
        ...
        try:
            ...
            self.log(f"获取 release 信息失败: {e}")
            return None

    def _compare_versions(self, v1, v2):
        """比较两个版本号，返回 1 表示 v1 > v2，0 相等，-1 小于"""
        def normalize(v):
    ...
    def prompt_update(self, latest_tag, release_info):
        """询问用户是否更新"""
        result = messagebox.askyesno(
            "发现新版本",
            f"发现新版本 {latest_tag}，当前版本 {VERSION_OF_LAUNCHER}。\n是否立即下载并更新？\n\n更新后启动器将自动重启。",
            parent=self.root
        )
        if result:
            self._perform_update(release_info)

    def _perform_update(self, release_info):
        """执行更新：下载新文件 → 启动更新脚本 → 退出当前程序"""
        # 获取当前可执行文件路径
        current_exe = sys.executable if getattr(sys, 'frozen', False) else None
        if not current_exe or not os.path.isfile(current_exe):
            self.log("无法定位当前程序文件，更新失败")
            return

        # 获取对应平台的下载 URL
        download_url = self._get_download_url_for_platform(release_info)
        if not download_url:
            self.log("未找到适合当前平台的更新文件")
            return

        self.log(f"开始下载更新: {download_url}")

        # 下载到临时文件
        try:
            temp_dir = tempfile.gettempdir()
            new_exe_path = os.path.join(temp_dir, f"launcher_update_{int(time.time())}.exe" if sys.platform == 'win32' else f"launcher_update_{int(time.time())}")
            ...
            with open(new_exe_path, 'wb') as f:
                for chunk in response.iter_content(chunk_size=8192):
                    ...
                    if total_size:
                        ...
                        self.log(f"下载进度: [{'#' * progress}{'.' * (50-progress)}] {downloaded//1024}/{total_size//1024} KB", keep_line=True)
            self.log("\n下载完成，准备更新...")

            # 构建更新脚本 / 批处理命令
            if sys.platform == 'win32':
                # Windows 使用批处理
                bat_path = os.path.join(temp_dir, "update_launcher.bat")
    ...
    ''')
                ...
                script_path = os.path.join(temp_dir, "update_launcher.sh")
                with open(script_path, 'w', encoding='utf-8') as sh:
                    sh.write(f'''#!/bin/bash
    sleep 1
    while fuser "{current_exe}" >/dev/null 2>&1; do sleep 1; done
    ...
    ''')
            ...
            self.log(f"更新失败: {e}")
            messagebox.showerror("更新失败", f"下载或安装更新时出错：{e}")

    def _get_download_url_for_platform(self, release_info):
        """根据当前操作系统和架构，从 release assets 中选择正确的下载链接"""
        system = platform.system().lower()
        arch = platform.machine().lower()
        ...
        # 期望的文件名模式，例如 SimpleMCLauncher-windows-amd64.exe
        target_name = f"SimpleMCLauncher-{os_key}-{arch_key}{ext}"
        # 也尝试无架构后缀的通用名
        alt_name = f"SimpleMCLauncher-{os_key}{ext}"

        assets = release_info.get('assets', [])
        for asset in assets:
            name = asset.get('name', '')
            if name == target_name or name == alt_name:
                return asset.get('browser_download_url')
        return None

    def log(self, message, keep_line=False):
        """修改原有的 log 方法以支持 keep_line 参数（可选，如果不想修改原方法，可保留原样）"""
        # 如果 keep_line=True，则不换行（用于进度条）
        if keep_line:
            self.root.after(0, lambda: self._log_safe_no_newline(message))
        else:
            self.root.after(0, lambda: self._log_safe(message))

    def _log_safe_no_newline(self, message):
        """不换行输出（用于进度条）"""
        self.console.config(state='normal')
        self.console.insert(tk.END, message)
        self.console.see(tk.END)
