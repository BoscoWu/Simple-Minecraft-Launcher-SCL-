...
class SimpleMCLauncher:
    ...
    def microsoft_login(self):
        """微软 OAuth 设备码登录（不依赖 minecraft-launcher-lib）"""
        self.log("正在启动微软设备码登录流程...")
        try:
            client_id = CLIENT_ID  # 使用你已有的 CLIENT_ID
            scope = "XboxLive.signin offline_access"

            # 1. 获取设备码
            device_url = "https://login.microsoftonline.com/consumers/oauth2/v2.0/devicecode"
            device_data = {"client_id": client_id, "scope": scope}
            ...
            self.log("=" * 50)
            self.log(f"请访问 {verification_uri} 并输入代码: {user_code}")
            self.log("=" * 50)
            ...
            resp = requests.post(xbl_url, json=xbl_body)
            resp.raise_for_status()
            xbl_data = resp.json()
            ...
            resp = requests.post(xsts_url, json=xsts_body)
            resp.raise_for_status()
            ...
            resp = requests.post(mc_auth_url, json=mc_auth_body)
            resp.raise_for_status()
            mc_token = resp.json()["access_token"]
            ...
            self.config["ms_uuid"] = uuid
            self.config["ms_access_token"] = mc_token
            self.save_config()

            self.log(f"✅ 登录成功！欢迎 {username}")
            self.root.after(0, lambda: self.log(f"当前玩家已切换为: {self.get_player_display()}"))
    ...
    def refresh_microsoft_token(self):
        ...
        try:
            ...
            # 重新获取 XSTS 和 Minecraft token（同登录流程）
            xbl_url = "https://user.auth.xboxlive.com/user/authenticate"
            xbl_body = {
                "Properties": {
                    "AuthMethod": "RPS",
                    "SiteName": "user.auth.xboxlive.com",
                    "RpsTicket": f"d={new_access_token}"
                },
                "RelyingParty": "http://auth.xboxlive.com",
                "TokenType": "JWT"
            }
            ...
            self.config["ms_refresh_token"] = new_refresh_token
            self.config["ms_username"] = username
            self.config["ms_uuid"] = uuid
            self.config["ms_access_token"] = mc_token
            self.save_config()
    ...
    def check_for_updates(self):
        ...
        try:
            ...
            current_version = VERSION_OF_LAUNCHER.lstrip('v')

            if latest_version == current_version:
                self.log(f"当前已是最新版本 {current_version}")
                return

            self.log(f"发现新版本: {latest_version} (当前: {current_version})")

            answer = messagebox.askyesno(
                "发现新版本",
                f"最新版本为 {latest_version}，是否立即更新？\n\n更新将自动下载并重启启动器。"
            )
            if not answer:
                self.log("用户取消更新")
                return
            ...
            for asset in assets:
                if asset["name"].endswith(suffix):
                    ...
                    break
            if not asset_url:
                self.log("未找到适合当前系统的更新文件，请手动下载。")
                return

            current_exe = sys.executable if getattr(sys, 'frozen', False) else __file__
            current_dir = os.path.dirname(current_exe)
            new_file = os.path.join(current_dir, f"launcher_new{suffix}")
            ...
            self._replace_and_restart(current_exe, new_file)

        except Exception as e:
            self.log(f"检查更新失败: {e}")
            self.log(traceback.format_exc())

    def _replace_and_restart(self, old_path, new_path):
        if os.name == 'nt':
    ...
    rm -- "$0"
    """
            script_path = os.path.join(os.path.dirname(old_path), "update_script.sh")
            with open(script_path, 'w') as f:
                f.write(script_content)
            os.chmod(script_path, 0o755)
            subprocess.Popen([script_path], shell=True)
        self.log("正在更新，启动器将重启...")
        self.root.quit()
        sys.exit(0)
